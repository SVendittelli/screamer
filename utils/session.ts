...
/**
 * Used in frontend routes to get session information when rendering on the server side.
 *
...
/**
 * Used as a session guard for each of our API routes. If a session exists it will be passed to the callback, which is where we will write our API logic. If no session exists it will pass undefined to the callback
 *
